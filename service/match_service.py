...
class MatchManager(ABC):
    ...
    def finalize_setup(self):
        """When the last player has been registered and the
        match is ready to go, do some final initialisation
        Note: I'm still not happy the way this works (has to do with
        darts_match as a generic domain object versus match management
        here abstracted and then implemented in concrete match classes)
        :return
        """
        self.post_init()
